...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const DataService = {
    ...
    async fetchBlog() {
      const defaults = [
        { id: 1, title: 'Jak probíhá čištění fasád?', summary: 'Přečtěte si, jaké technologie používáme pro hloubkovou očistu vašeho domu.', date: '2026-04-10', status: 'Publikováno' },
        { id: 2, title: 'Výhody nano-ochrany povrchů', summary: 'Proč je nano-technologie revolucí v údržbě nemovitostí?', date: '2026-04-05', status: 'Publikováno' }
      ];
  ...
  const renderBlog = (posts) => `
    <div class="admin-header">
      ...
      <button class="admin-btn-primary" id="add-post-btn" style="width: auto; padding: 0.75rem 1.5rem;">+ Nový článek</button>
    </div>
    <div class="admin-card">
      <table class="admin-table">
        ...
        <tbody>
          ${posts.map(p => `
            <tr>
              ...
              <td>${p.date}</td>
              <td><span class="admin-badge" style="background: #dcfce7; color: #166534">Publikováno</span></td>
              <td><button onclick="deletePost(${p.id})" style="background: none; border: none; cursor: pointer; color: #ef4444;">🗑️</button></td>
            </tr>
  ...
  window.deletePost = async (id) => {
    ...
    await DataService.saveBlog(filtered);
    renderSection('blog');
  };
