...
<<<<<<< HEAD
                ...
                import('./supabase-config.js')
                  ...
                  .catch(err => console.error('Supabase CDN or network error, fallback used:', err));

                // Fallback for offline/local view
                const leads = JSON.parse(localStorage.getItem('nanofusion_leads') || '[]');
                leads.unshift({ id: Date.now(), name: userData.contact, service: userData.service, priceRange: rangeStr, status: 'Nová' });
...
<<<<<<< HEAD
    const playNotificationSound = () => {
        try {
            ...
            console.log("Audio zablokováno prohlížečem.");
        }
    };
    ...
    setTimeout(() => {
        if (!chatOpenedByUser) {
            ...
            chatWindow.classList.add('active-pop');
            if (chatState === 'INIT') {
                startChat();
            }
            console.log('AI Chat: Automatické otevření pro zvýšení konverze.');
        }
    }, 4000); // 4 sekundy po načtení
};

// Spuštění po inicializaci DOM
const runSafe = () => {
    if (document.body) {
        initChat();
    } else {
